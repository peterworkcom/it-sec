...
- `DL` has enabled the emergence of technologies like `LLM`s (and other generative `AI`), which, through the use of `transformer neural networks` and attention, can be queried in natural language, understand it and respond in a human-like, conversational fashion.
